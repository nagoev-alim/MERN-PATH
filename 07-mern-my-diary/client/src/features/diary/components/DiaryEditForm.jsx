...
const DiaryEditForm = () => {
  ...
  useEffect(() => {
    ...
    dispatch(API_DIARY.READ_SINGLE(id))
      .then(() => {
        resetDiaryState();
        entry.date && setValue('date', new Date(entry.date).toISOString().substring(0, 10));
        setValue('day', entry.day);
        setValue('weather', entry.weather);
        setValue('social', entry.social);
        setValue('school', entry.school);
        setValue('romance', entry.romance);
        setValue('other', entry.other);
        setValue('meals', entry.meals);
        setValue('health', entry.health);
        setValue('chores', entry.chores);
        setValue('beauty', entry.beauty);
        setValue('work', entry.work);
        setValue('emotions', entry.emotions);
        setValue('hobbies', entry.hobbies);
        setValue('events', entry.events);
        setValue('sleep', entry.sleep);
  ...
  return <form className='form' onSubmit={handleSubmit(onSubmit)}>
    <h3 className='font-medium text-xl'>✏️ Update record</h3>
    <FormGroup type='date' name='date' register={register}
    ...
    <label className='form-group'>
      ...
      <img src={entry.poster ??`/assets/images/cover.webp`} alt='photo' />
    </label>
